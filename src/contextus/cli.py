"""
Contextus CLI - Main entry point for the Model Context Protocol Manager CLI
"""
...
from contextus import __version__
from contextus.clients.client_config import ClientConfigManager
from contextus.commands import (
    add,
    client,
    config,
    custom,
...
from contextus.commands.share import share
...
def main(ctx, help_flag, version):
    """Contextus - Model Context Protocol Manager.
    ...
    if (
        ...
        if not active_target:
            ...
            console.print("Please run 'contextus target set <target>' to set an active target\n")

            # Show available clients
            from contextus.clients.client_registry import ClientRegistry

            console.print("[bold green]Available Clients, set one with 'contextus target set @<client>':[/]")
            for client in ClientRegistry.get_supported_clients():
                console.print(f"  - {client}")

            from contextus.profile.profile_config import ProfileConfigManager

            # Show available profiles
            console.print("[bold green]Available Profiles, set one with 'contextus target set %<profile>':[/]")
    ...
    if ctx.invoked_subcommand is None or help_flag:
        ...
        if active_target:
            ...
            console.print("[bold red]No active target set![/] Please run 'contextus target set <target>' to set one.")
        console.print("")

        # Display usage info
        console.print("[bold green]Usage:[/] [white]contextus [OPTIONS] COMMAND [ARGS]...[/]")
        console.print("")
        ...
        commands_table.add_row("  [cyan]target[/]", "Manage the active Contextus target.")

        commands_table.add_row("[yellow]client[/]")
        commands_table.add_row("  [cyan]client[/]", "Manage supported Contextus clients.")
        ...
        commands_table.add_row("  [cyan]profile[/]", "Manage Contextus profiles.")
        ...
        commands_table.add_row("  [cyan]config[/]", "Manage Contextus configuration.")
        commands_table.add_row("  [cyan]inspector[/]", "Launch the Contextus Inspector UI to examine servers.")
        console.print(commands_table)

        # Additional helpful information
        console.print("")
        console.print("[italic]Run [bold]contextus COMMAND -h[/] for more information on a command.[/]")
