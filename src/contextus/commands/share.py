"""
Share command for Contextus - Share a single MCP server through a tunnel
"""
...
from contextus.utils.config import DEFAULT_SHARE_ADDRESS
...
@click.option("--address", type=str, default=None, help="Remote address for tunnel, use share.contextus.sh if not specified")
@click.option(
...
def share(command, port, address, http, timeout, retry):
    ...
    \b
        contextus share "uvx mcp-server-fetch"
        contextus share "npx mcp-server" --port 5000
        contextus share "uv run my-mcp-server" --address myserver.com:7000
        contextus share "npx -y @modelcontextprotocol/server-everything" --retry 3
    """
