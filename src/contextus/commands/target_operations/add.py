...
from contextus.commands.target_operations.common import (
    client_add_profile,
    client_add_server,
    determine_scope,
    profile_add_server,
...
from contextus.schemas.full_server_config import FullServerConfig
from contextus.utils.repository import RepositoryManager
from contextus.utils.scope import PROFILE_PREFIX, ScopeType
...
def add(server_name, force=False, alias=None, target: str | None = None):
    ...
    \b
        contextus add time
        contextus add everything --force
        contextus add youtube --alias yt
        contextus add youtube --target %myprofile
        contextus add %profile --target @windsurf
    """
    ...
    base_dir = os.path.expanduser("~/.contextus")
    ...
    if has_non_standard_argument_define:
        ...
        console.print(
            ...
            "[bold cyan]Future versions of Contextus will standardize all arguments in server configuration to use ${VARIABLE_NAME} format.[/]\n"
