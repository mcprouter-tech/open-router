"""
Contextus Inspector command for examining MCP servers through a user interface
"""
...
def inspector(args, yes):
    """Launch the Contextus Inspector UI to examine servers.
    ...
    Launch the Inspector with custom arguments:
        contextus inspector npx my-server --port 3000

    Use with NPM packages:
        contextus inspector npx server-postgres

    Use with Python packages:
        contextus inspector uvx mcp-server-git
    """
    # Show header
    console.print(
        Panel.fit("[bold green]Contextus Inspector[/]\nModel Context Protocol Inspection Tool", border_style="cyan")
    )
    ...
    try:
        ...
        if args:
            ...
            console.print(f"[bold]Running Contextus Inspector with arguments:[/] {arg_string}")
        else:
            # No arguments provided, prompt for confirmation
            if not yes:
                click.echo("\nStarting Contextus Inspector with no arguments.")
        ...
        console.print("[cyan]Starting Contextus Inspector...[/]")
        ...
        try:
            ...
            try:
                ...
                console.print("[bold green]Starting Contextus Inspector...[/]")
...
def show_inspector_help():
    ...
    console.print(
        Panel.fit(
            ...
            title="Contextus Inspector Help",
            border_style="cyan",
        )
    )
    ...
    console.print("  [cyan]contextus inspector filesystem[/]")
    console.print("    Launch Inspector for an installed server")
    console.print('\n  [cyan]contextus inspector --package server-postgres --package-args "postgres://127.0.0.1/testdb"[/]')
    console.print("    Inspect an NPM package with arguments")
    console.print("\n  [cyan]contextus inspector --python --package mcp-server-git[/]")
