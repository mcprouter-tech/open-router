...
from contextus.core.schema import STDIOServerConfig
...
def add(profile, force=False):
    ...
    console.print(f"You can now add servers to this profile with 'contextus add --target %{profile} <server_name>'\n")
    console.print(
        f"Or apply existing config to this profile with 'contextus profile apply {profile} --server <server_name>'\n"
    )
