...
from contextus.utils.scope import ScopeType, extract_from_scope, format_scope
...
def target():
    """Manage Contextus working target."""
    pass
...
def set_target(target):
    """Set the active Contextus working target.

    TARGET is the name of the client or profile to set as active.
    Examples:

    \b
        contextus target set @windsurf
        contextus target set %profile_dev
    """
    ...
    if success:
        ...
        panel = Panel(
            ...
            f"Commands like 'contextus ls', 'contextus add', 'contextus rm', 'contextus stash', and 'contextus pop' will now operate on {scope_name}.",
