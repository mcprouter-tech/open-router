...
from contextus.core.schema import ServerConfig
from contextus.monitor.base import AccessEventType, AccessMonitor
from contextus.monitor.event import trace_event
from contextus.profile.profile_config import ProfileConfigManager
from contextus.utils.config import (
    CONTEXTUS_AUTH_HEADER,
    CONTEXTUS_PROFILE_HEADER,
    PROMPT_SPLITOR,
    RESOURCE_SPLITOR,
...
from contextus.utils.errlog_manager import ServerErrorLogManager
...
class MCPRouter:
    ...
    def _create_aggregated_server(self) -> server.Server[object]:
        ...
        """
        app: server.Server[object] = server.Server(name="contextus-router")
...
class AuthMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: t.Callable[[Request], t.Awaitable[Response]]):
        auth_header = request.headers.get(CONTEXTUS_AUTH_HEADER)
...
class ProfileMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next: t.Callable[[Request], t.Awaitable[Response]]):
        profile = request.headers.get(CONTEXTUS_PROFILE_HEADER)
